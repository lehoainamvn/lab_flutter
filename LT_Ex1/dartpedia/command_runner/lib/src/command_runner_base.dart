...
class CommandRunner {
  // Thêm onOutput vào constructor
  CommandRunner({this.onOutput, this.onError});

  // Khai báo hàm callback onOutput
  ...
  Future<void> run(List<String> input) async {
    try {
      ...
      if (results.command != null) {
        ...
        // SỬ DỤNG ONOUTPUT Ở ĐÂY
        if (onOutput != null) {
          await onOutput!(output.toString());
        } else {
          print(output.toString()); // Mặc định nếu không truyền gì
        }
      }
  ...
  /// Hàm phân tích cú pháp (Đã được nâng cấp để bắt lỗi)
  ArgResults parse(List<String> input) {
    ArgResults results = ArgResults();
    if (input.isEmpty) return results;

    // 1. Kiểm tra lệnh đầu tiên có hợp lệ không
    ...
    // 2. Kiểm tra xem người dùng có lỡ gõ 2 lệnh cùng lúc không
    if (results.command != null &&
        input.isNotEmpty &&
    ...
    while (i < input.length) {
      if (input[i].startsWith('-')) {
        ...
        // 3. Kiểm tra xem option người dùng gõ có nằm trong danh sách hỗ trợ không
        ...
        if (option.type == OptionType.option) {
          // 4. Kiểm tra xem option cần giá trị truyền vào nhưng bị bỏ trống không
          if (i + 1 >= input.length) {
            throw ArgumentException(
        ...
        // 5. Kiểm tra xem người dùng có gõ quá 1 đối số vị trí không
