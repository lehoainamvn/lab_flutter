...
/// Lớp trừu tượng làm khung cho mọi loại tham số
...
/// Lớp đại diện cho Option hoặc Flag
...
/// Lớp trừu tượng đại diện cho một Lệnh (Command)
...
/// Lớp chứa kết quả sau khi phân tích cú pháp
class ArgResults {
  Command? command;
  String? commandArg;
  ...
  bool flag(String name) {
    for (var option in options.keys.where((option) => option.type == OptionType.flag)) {
      if (option.name == name) return options[option] as bool;
    }
    return false;
  }

  bool hasOption(String name) => options.keys.any((option) => option.name == name);
